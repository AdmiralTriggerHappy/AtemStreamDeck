...
#define WM_MIX_EFFECT_BLOCK_PROGRAM_INPUT_CHANGED				L"Program Input Changed"
#define WM_MIX_EFFECT_BLOCK_PREVIEW_INPUT_CHANGED				L"Preview Input Changed"
#define WM_MIX_EFFECT_BLOCK_IN_TRANSITION_CHANGED				L"In Tranistion Changed"
#define WM_MIX_EFFECT_BLOCK_TRANSITION_POSITION_CHANGED			L"Tranistion Postion Changed"
#define WM_MIX_EFFECT_BLOCK_TRANSITION_FRAMES_REMAINING_CHANGED	L"Tranistion Frames Remaining Changed"
#define WM_MIX_EFFECT_BLOCK_FTB_FRAMES_REMAINING_CHANGED		L"FTB Remaining Changed"
#define WM_SWITCHER_INPUT_LONGNAME_CHANGED						L"Input Name Changed"
#define WM_SWITCHER_DISCONNECTED								L"Disconnected"
...
list<AtemControl::inputDetails> AtemControl::getListInputList()
{
	//BMDSwitcherInputId* currentInput;
	return inputList;
...
HRESULT AtemControl::SetPreview(BMDSwitcherInputId _inputID)
{
	return mMixEffectBlock->SetPreviewInput(_inputID);
}

HRESULT AtemControl::SetProgram(BMDSwitcherInputId _inputID)
{
	return mMixEffectBlock->SetProgramInput(_inputID);
}
...
HRESULT AtemControl::switcherConnected()
{
	...
	if (SUCCEEDED(result))
	{
		...
		while (S_OK == inputIterator->Next(&input))
		{
			...
			inputDetails inputDetailStore;
			//BSTR* InputName = "";
			//input->GetShortName(InputName);

			if (SUCCEEDED(input->GetInputId(&inputID)))
			{
				inputDetailStore.inputID = inputID;
			}

			if (SUCCEEDED(input->GetShortName(&shortName)))
			{

				inputDetailStore.shortName = _bstr_t(shortName);
			}

			if (SUCCEEDED(input->GetShortName(&LongName)))
			{
				inputDetailStore.LongName = _bstr_t(LongName);
			}

			if (SUCCEEDED(input->GetPortType(&portType)))
			{
				inputDetailStore.portType = portType;
				switch (portType)
				{
				case bmdSwitcherPortTypeExternal:
					inputDetailStore.HRPortType = L"External";
					break;

				case bmdSwitcherPortTypeBlack:
					inputDetailStore.HRPortType = L"Black";
					break;

				case bmdSwitcherPortTypeColorBars:
					inputDetailStore.HRPortType = L"Colour Bars";
					break;

				case bmdSwitcherPortTypeColorGenerator:
					inputDetailStore.HRPortType = L"Colour Generator";
					break;

				case bmdSwitcherPortTypeMediaPlayerFill:
					inputDetailStore.HRPortType = L"Media Player Fill";
					break;

				case bmdSwitcherPortTypeMediaPlayerCut:
					inputDetailStore.HRPortType = L"Media Player Cut";
					break;

				case bmdSwitcherPortTypeSuperSource:
					inputDetailStore.HRPortType = L"Super Source";
					break;

				case bmdSwitcherPortTypeMixEffectBlockOutput:
					inputDetailStore.HRPortType = L"Mix Effect Block Output";
					break;

				case bmdSwitcherPortTypeAuxOutput:
					inputDetailStore.HRPortType = L"Aux Output";
					break;

				case bmdSwitcherPortTypeKeyCutOutput:
					inputDetailStore.HRPortType = L"Key Cut Output";
					break;

				case bmdSwitcherPortTypeMultiview:
					inputDetailStore.HRPortType = L"MultiView";
					break;

				case bmdSwitcherPortTypeExternalDirect:
					inputDetailStore.HRPortType = L"External Direct";
					break;
			...
			inputList.push_back(inputDetailStore);
			input->Release();
...
wstring AtemControl::GetProductName()
{
	if (mMixerConnected)
	{
		//return W2A(mProductName);

		//return std(mProductName, mProductName.GetLength());
	//std::wstring wideName = wstring(mProductName.GetString());
		mProductName.GetString();
		std::wstring MixerName(mProductName);

		return MixerName;

	}
	else {
		return L"Atem Not Connected";
	}
}
